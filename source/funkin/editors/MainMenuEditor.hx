...
class MainMenuEditor extends MusicBeatState
{
        ...
        // Stored reference so we can safely remove the listener on destroy.
        var keyDownHandler:KeyCode -> KeyModifier -> Void;
        ...
        override public function create()
        {
                ...
                // Route all native key events to the focused text box so the user can actually type.
                // The previous implementation only dispatched a handful of special keys (BACKSPACE,
                // DELETE, LEFT, RIGHT, ENTER, Ctrl+C/V/X) and never forwarded printable characters,
                // which is why nothing could be typed into the text boxes.
                keyDownHandler = windowKeyDown;
                if (FlxG.stage != null
                        && FlxG.stage.application != null
                        && FlxG.stage.application.window != null)
                {
                        FlxG.stage.application.window.onKeyDown.add(keyDownHandler);
                }

                // Printable characters arrive through a SEPARATE callback.
                // Without this, only special keys (arrows, backspace, etc.)
                // would reach the text box and normal typing would be impossible.
                textInputHandler = windowTextInput;
                if (FlxG.stage != null
                        && FlxG.stage.application != null
                        && FlxG.stage.application.window != null)
                {
                        FlxG.stage.application.window.onTextInput.add(textInputHandler);
                }
                ...
                // Defensive: make sure the options array always exists so later
                // .length / .push / .splice calls don't null-ref.
                if (menuJson.options == null) menuJson.options = [];
                ...
                if (menuJson.background != null && menuJson.background != "")
                {
                        ...
                        {
                                trace("Failed to load background '" + menuJson.background + "': " + e);
                        }
                }
                ...
                loadBgBtn = new FlxButton(175, 100, "Load BG", function() {
                        ...
                        if (labelText != null && labelText.trim() != "") {
                                ...
                                try
                                {
                                        ...
                                        {
                                                trace("Load BG: image '" + newBgKey + "' not found.");
                                        }
                                }
                                catch (e:Dynamic)
                                {
                                        trace("Load BG failed for '" + newBgKey + "': " + e);
                                }
                        }
                });
        ...
        {
                // Drop focus first so the text box cleans up any internal state.
                clearTextBoxFocus();

                // Make sure we don't leak the window key listener into the next state.
                if (keyDownHandler != null
                        && FlxG.stage != null
                        && FlxG.stage.application != null
                        && FlxG.stage.application.window != null)
                {
                        FlxG.stage.application.window.onKeyDown.remove(keyDownHandler);
                }
                keyDownHandler = null;
                ...
                if (textInputHandler != null
                        ...
                        && FlxG.stage.application.window != null)
                {
                        FlxG.stage.application.window.onTextInput.remove(textInputHandler);
                }
        ...
        /**
         * Native window key handler. Forwards every key press (including
         * printable characters, which the old manual dispatcher missed) to
         * the currently focused text box.
         */
        function windowKeyDown(keyCode:KeyCode, modifier:KeyModifier):Void
        {
                // Don't dispatch while a substate (HELP / Reset) is open — its
                // own input handling should take priority.
                if (subState != null) return;

                // We check `currentTextBox != null` instead of `currentTextBox.focused`
                // because `focused` goes through UIState.state.focusedElement, which is
                // always null when this editor lives in a MusicBeatState rather than
                // a UIState.  The `externallyFocused` flag on UITextBox handles the
                // visual side (caret + selected frame).
                if (currentTextBox == null || !currentTextBox.visible)
                        return;

                // Unfocus on Escape so the update() loop can safely switch state.
                if (keyCode == ESCAPE) {
                        clearTextBoxFocus();
                        return;
                }
                ...
                try {
                        currentTextBox.onKeyDown(keyCode, modifier);
                } catch (e:Dynamic) {
                        // UIState.playEditorSound() in the default case can crash when
                        // UIState.state is null.  Swallow so it doesn't take the whole
                        // editor down — the character will still arrive via onTextInput.
                }

                // ENTER / RETURN commits and unfocuses the box, matching the
                // original behavior.
                if (keyCode == RETURN)
                {
                        currentTextBox.externallyFocused = false;
        ...
        /**
         * Native window text-input handler.  This is the ONLY way printable
         * characters (a–z, 0–9, symbols, …) reach the text box.  `onKeyDown`
         * only deals with special keys (arrows, backspace, shortcuts).
         */
        function windowTextInput(text:String):Void
        {
                if (subState != null) return;
                if (currentTextBox == null || !currentTextBox.visible) return;
                currentTextBox.onTextInput(text);
        }

        function clearTextBoxFocus():Void
        {
        ...
        {
                menuItems.clear();
                if (menuJson == null || menuJson.options == null) return;
                ...
                for (i in 0...optionShit.length)
                {
                        ...
                        // Atlas load can fail if the asset is missing — don't take the
                        ...
                        {
                                trace("Failed to load atlas 'mainmenu/menu_" + optionShit[i] + "': " + e);
                        }
        ...
        override public function update(elapsed:Float)
        {
                ...
                // Click-to-focus handling for the four text boxes.
                if (FlxG.mouse.justPressed)
                {
                        ...
                        for (box in boxes)
                        {
                                if (box != null && box.visible && FlxG.mouse.overlaps(box))
                                {
                                        clickedBox = true;
                                        if (currentTextBox != box)
                                        {
                                                ...
                                                currentTextBox = box;
                                                currentTextBox.externallyFocused = true;
                ...
                if (FlxG.keys.justPressed.F1)
                {
                        ...
                        // Hiding the panel shouldn't leave an invisible text box focused.
                        if (!panelVisible) clearTextBoxFocus();
                }
        }
